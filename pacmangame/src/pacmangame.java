...
class pacman {
	...
	public pacman(){
		...
		class KListener extends KeyAdapter{  //키어댑터를 상속받아 인터페이스를 구현했을때보다 더 쉽게 원하는 메소드만 불러올수있음.
			public void keyPressed(KeyEvent e) {
				if(numOfDot<1) {//점의 남은 개수가 없어지면 성공 프레임을 만들어줌.
					frame.dispose();
					t.stop();
					GameOverFrame gameOverFrame = new GameOverFrame(opt);
		...
		TimeListener t1Listner = new TimeListener();



		t = new Timer(150, tListener);//0.15초에 한번씩 랜덤값에 의한 적의 움직임 설정.
		t.start();
		t1= new Timer(1000,t1Listner); //1초에 한번씩 카운트가 증가함 즉, 카운트 는 1초당 1씩 올라가다가 끝~
		t1.start();
...
public class pacmangame { 
	...
	public static void main(String[] args) {
		...
		opt.af = new PasswordFrame(opt);
		opt.df= new DeleteFrame(opt);
		opt.gof= new GameOverFrame(opt);
